...
class ProfileTestCase(TestCase):
        ...
        def test_valid_profile_detail(self):
            ...
            self.assertEquals(resp_json["name"] , "Cathy")
        ...
        def test_valid_create_profile(self):
            ...
            resp_json = json.loads((response.content).decode("utf-8"))
            self.assertEquals(resp_json["name"] , "Dan")
        ...
        def test_valid_edit_profile(self):
            response = self.client.post(reverse('retrieve_update_profiles', args=[self.test_profile.id]), {'name': "notCathy"})
            resp_json = json.loads((response.content).decode("utf-8"))
            self.assertEquals(resp_json["name"] , "notCathy")

        def test_invalid_edit_profile(self):
            wrongdata = {"name": "test_invalid_edit"}
